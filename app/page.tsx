...
import { shuffleArray, calculateResults } from "../utils/helpers";
import { playSound, setSoundEnabled } from "../utils/sounds";
import { playAmbiance, stopAmbiance, setAmbianceSoundEnabled } from "../utils/ambiance";
import { IMAGE_PLACEHOLDER, TRANSITION_DURATION } from "../utils/constants";
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      const storedTheme = localStorage.getItem("quiz-dark-mode");
      if (storedTheme !== null) {
        setIsDark(storedTheme === 'true');
      }
      const storedSound = localStorage.getItem("quiz-sound-enabled");
      if (storedSound !== null) {
        const sound = storedSound === 'true';
        setSoundEnabledState(sound);
        setSoundEnabled(sound);
        setAmbianceSoundEnabled(sound);
      }
    } catch (e) {
  ...
  const toggleSound = () => {
    ...
    setAmbianceSoundEnabled(newState);
    if(newState && started && !showResults && shuffledQuestions.length > 0) {
      playAmbiance(shuffledQuestions[currentQuestion].theme);
    } else {
      stopAmbiance();
    }
    try {
  ...
  if (showResults) {
    ...
    return (
      <Results
        ...
        onShare={async () => console.log('share')}
